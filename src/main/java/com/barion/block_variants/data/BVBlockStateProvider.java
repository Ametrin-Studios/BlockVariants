        ...
        for(B block : blocks) {
            ...
            if(block instanceof StairsBlock){
                ...
                if(BVUtil.shouldAppendS(name)) {texture = blockTexture(name.replace("_stairs", "s"));}
                    ...
                    stairsBlock((StairsBlock) block, blockTexture("chiseled_quartz_block"), blockTexture("chiseled_quartz_block_top"), blockTexture("chiseled_quartz_block_top"));
                    continue;
                }else if(name.contains("smooth_stone")) {
                    stairsBlock((StairsBlock) block, blockTexture("smooth_stone_slab_side"), blockTexture("smooth_stone"), blockTexture("smooth_stone"));
                    continue;
                }else if(BVUtil.isCutSandstone(name)) {
                    texture = blockTexture(name.replace("cut_", "").replace("stairs", "top"));
                    stairsBlock((StairsBlock) block, blockTexture(name.replace("_stairs", "")), texture, texture);
                    continue;
                }else if(BVUtil.isBasalt(name)) {
                    texture = blockTexture(name.replace("stairs", "top"));
                    stairsBlock((StairsBlock) block, blockTexture(name.replace("stairs", "side")), texture, texture);
                    continue;
